...
const ThreeScene = () => {
  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      (gltf) => {
        ...
        // Marquer la fin du chargement et permettre l'animation
        setIsLoading(false);
      ...
      undefined, // Option de suivi de progression du chargement (facultatif)
      (error) => {
        console.error('Error loading model:', error);
        setIsLoading(false);  // Si une erreur se produit, on sort du mode de chargement
      }
    );
    ...
    const animate = () => {
      ...
      if (modelRef.current) {
        ...
        // Ajouter une rotation très lente autour de l'axe Y (vous pouvez ajuster la valeur)
        modelRef.current.rotation.y += 0.001;  // Rotation lente autour de l'axe Y
        modelRef.current.rotation.x += 0.0005; // Rotation lente autour de l'axe X (optionnel)
      }
  ...
  useEffect(() => {
    if (modelRef.current) {
      if (isModalOpen) {
        ...
        targetPosition.current.set(-3, 0, 0); // Déplacer davantage à gauche
      } else {
  ...
  return (
    <>
      {/* Page de chargement */}
      {isLoading && (
        <div className="loading-page" style={{ display: 'block' }}>
